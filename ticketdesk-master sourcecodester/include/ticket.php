...
class ticket {
	...
	public static function withParams($ticketParameters) {
		$instance = new self();
			...
			$instance->namaCustomer = $ticketParameters['namaCustomer'];
			$instance->noTelepon = $ticketParameters['noTelepon'];
			$instance->alamat = $ticketParameters['alamat'];
			$instance->sumber = $ticketParameters['sumber'];
     ...
     * Adds the ticket to the database
   	...
   	public function addTicket($isClosed) {
			$prep_stmt = "insert into tickets (clientid,user,subject,nama_customer,no_telepon,alamat,sumber,categoryid,subcategoryid,comments,transferYn,groupid,openDate,parentTicketId,assignedUser)" .
			 	     "values(?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?,?)";
			$parentTicketId = 0;
			if ($insert_stmt = $this->mysqli->prepare($prep_stmt)) {
				$insert_stmt->bind_param('issssssiisiiis',$this->clientId,
							$this->user,
							$this->subject,
							$this->namaCustomer,
							$this->noTelepon,
